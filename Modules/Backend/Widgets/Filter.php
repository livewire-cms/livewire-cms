...
class Filter extends WidgetBase
{
    ...
    public function renderScopeElement($scope)
    {
        ...
        switch ($scope->type) {
            ...
            case 'text':
                ...
                $params['size'] = \Arr::get($scope->config, 'size', 10);
    ...
    public function onFilterUpdate()
    {
        ...
        switch ($scope->type) {
            ...
            case 'text':
                $value = request('options.value.' . $scope->scopeName) ?: null;
                $this->setScopeValue($scope, $value);
                break;
        }
    ...
    public function onFilterGetOptions()
    {
        ...
        if (!$scopeName = request('scopeName')) {
            return;
        }
        ...
        switch ($scope->type) {
            ...
            default:
                ...
                if ($scopeConditions = $scope->conditions) {
                    ...
                    if (is_array($value)) {
                        $filtered = implode(',', \Arr::build($value, function ($key, $_value) {
    ...
    protected function optionsFromAjax($options)
    {
        ...
        foreach ($options as $option) {
            ...
            $processed[$id] = \Arr::get($option, 'name');
        }
        return $processed;
    }
