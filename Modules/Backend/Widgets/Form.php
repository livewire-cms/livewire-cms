...
use Modules\Backend\Classes\FormTabs;
...
class Form extends WidgetBase
{
    ...
    public function addFields(array $fields, $addToArea = null)
    {
        foreach ($fields as $name => $config) {
            ...
            $fieldTab = is_array($config) ? \Arr::get($config, 'tab') : null;
            if($name=='categories'){
                // dd($config,$fieldObj,$this);
            }
    ...
    protected function isFormWidget($fieldType)
    {
       ...
       if($fieldType=='fileupload'){
            // dd($this->widgetManager,$fieldType,$widgetClass);
        ...
        if (is_subclass_of($widgetClass, 'Modules\Backend\Classes\FormWidgetBase')) {
            return true;
        }
    ...
    protected function makeFormFieldWidget($field)
    {
        ...
        $widgetConfig->alias = $this->alias . \Str::studly($this->nameToId($field->fieldName));
    ...
    /**
     * Variant to \Arr::get() but preserves dots in key names.
     *
     * @param array $array
     * @param array $parts
     * @param null $default
