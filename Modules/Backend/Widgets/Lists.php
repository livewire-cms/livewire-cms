...
{
    use \Modules\Backend\Traits\PreferenceMaker;
    ...
    public function init()
    {
        ...
        // dd(312312321);
        // dd(31312231,$this->recordsPerPage);
    ...
    public function prepareQuery()
    {
        ...
        $query->where(function ($innerQuery) use ($primarySearchable, $relationSearchable, $joins) {
            ...
            if ($joins) {
                foreach (array_unique($joins) as $join) {
                     ...
                     */
                    $columnsToSearch = \Arr::get($relationSearchable, $join, []);
        ...
        if (($sortColumn = $this->getSortColumn()) && !$this->showTree && in_array($sortColumn, array_keys($this->getVisibleColumns()))) {
            if (($column = \Arr::get($this->allColumns, $sortColumn)) && $column->valueFrom) {
    ...
    protected function makeListColumn($name, $config)
    {
         ...
         */
        if (\Str::startsWith($name, 'pivot[') && strpos($name, ']') !== false) {
    ...
    public function getColumnValueRaw($record, $column)
    {
        ...
        if ($column->valueFrom && $column->relation) {
            ...
            // dd($column,$column->valueFrom,$columnName,$record->getRelations());
    ...
    public function getColumnValue($record, $column)
    {
        ...
        if($column->columnName=='users_count'){
            // dd($value);
        }

        if (method_exists($this, 'eval'. \Str::studly($column->type) .'TypeValue')) {
            if($column->columnName=='users_count'){
                // dd($column);
            }
            $value = $this->{'eval'. \Str::studly($column->type) .'TypeValue'}($record, $column, $value);
        }
        else {
