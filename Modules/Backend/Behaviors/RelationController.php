<?php namespace Modules\Backend\Behaviors;

use DB;
use Lang;
use Request;
use Form as FormHelper;
use Modules\Backend\Classes\ControllerBehavior;
...
class RelationController extends ControllerBehavior
{
    ...
    /**
     * @var const requestback parameter for the active relationship field.
     */
    const PARAM_FIELD = '_relation_field';

    /**
     * @var const requestback parameter for the active management mode.
     */
    const PARAM_MODE = '_relation_mode';

    /**
     * @var const requestback parameter for read only mode.
     */
    ...
    {
        // dd($this->model,$field,request(self::PARAM_FIELD));
        $field = $field ?: request(self::PARAM_FIELD);
        // dd($this->model,$field,request(self::PARAM_FIELD),$this->field,$this->manageMode);
    ...
    public function initRelation($model, $field = null)
    {
        ...
        if ($field == null) {
            $field = request(self::PARAM_FIELD);
        }
        ...
        if ($extraConfig = request(self::PARAM_EXTRA_CONFIG)) {
            $this->applyExtraConfig($extraConfig);
        }

        $this->alias = \Str::camel('relation ' . $field);
        ...
        $this->manageId = request('manage_id');
        $this->foreignId = request('foreign_id');
    ...
    public function relationRender($field, $options = [])
    {
        ...
        $extraConfig = \Arr::only($options, $allowConfig);
...
// dd(333);
    ...
    public function relationGetSessionKey($force = false)
    {
        ...
        if (request('_relation_session_key')) {
            return $this->sessionKey = request('_relation_session_key');
        }
    ...
    protected function makeFilterWidget($type)
    {
        ...
        $filterWidget = $this->makeWidget('Modules\Backend\Widgets\Filter', $filterConfig);
    ...
    protected function makeToolbarWidget()
    {
        ...
        $toolbarWidget = $this->makeWidget('Modules\Backend\Widgets\Toolbar', $toolbarConfig);
    ...
    protected function makeSearchWidget()
    {
        ...
        $widget = $this->makeWidget('Modules\Backend\Widgets\Search', $config);
    ...
    protected function makeViewWidget()
    {
        ...
        if ($this->viewMode == 'multi') {
            ...
            $widget = $this->makeWidget('Modules\Backend\Widgets\Lists', $config);
        ...
        elseif ($this->viewMode == 'single') {
            ...
            $widget = $this->makeWidget('Modules\Backend\Widgets\Form', $config);
    ...
    protected function makePivotWidget()
    {
        ...
        return $this->makeWidget('\Modules\Backend\Widgets\Form', $config);
    }
    ...
    public function onRelationManageForm()
    {
        ...
        $this->vars['newSessionKey'] = \Str::random(40);
    ...
    public function onRelationManageUpdate()
    {
        ...
        $this->beforeAjax();
        $saveData = $this->manageWidget->getSaveData();
        // dd($this->viewMode,$saveData);
        if ($this->viewMode == 'multi') {
            $model = $this->manageWidget->model;
            $modelsToSave = $this->prepareModelsToSave($model, $saveData);
            // dd($this->viewMode,$saveData,$model,$modelsToSave);
    ...
    public function onRelationManageDelete()
    {
         ...
         */
        if ($this->viewMode == 'multi') {
            if (($checkedIds = request('checked')) && is_array($checkedIds)) {
    ...
    public function onRelationManageAdd()
    {
         ...
         */
        if ($this->viewMode == 'multi') {
            $checkedIds = $recordId ? [$recordId] : request('checked');
    ...
    public function onRelationManagePivotForm()
    {
        ...
        $this->vars['foreignId'] = $this->foreignId ?: request('checked');
    ...
    protected function evalManageMode()
    {
        ...
        // dd(request(self::PARAM_MODE),$this->eventTarget,$this->relationType);

        // dd(request(self::PARAM_MODE),222);

        if ($mode = request(self::PARAM_MODE)) {
            return $mode;
        }
    ...
    protected function evalFormContext($mode = 'manage', $exists = false)
    {
        ...
        if (($context = \Arr::get($config, 'context')) && is_array($context)) {
            $context = $exists
                ? \Arr::get($context, 'update')
                : \Arr::get($context, 'create');
        }
    ...
    protected function applyExtraConfig($config, $field = null)
    {
        ...
        $parsedConfig = \Arr::only($config, ['readOnly']);
        $parsedConfig['view'] = \Arr::only($config, ['recordUrl', 'recordOnClick']);
