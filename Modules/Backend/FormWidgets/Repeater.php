...
class Repeater extends FormWidgetBase
{
    ...
    protected function processItems()
    {
        ...
        collect($currentValue)->each(function ($value, $index) {
            $this->makeItemFormWidget($index, \Arr::get($value, '_group', null));
        });
    }
    ...
    protected function getNextIndex()
    {
        if ($this->loaded === true) {
            ...
            // dd(post()->all(),$data, $this->formField->getName());
     ...
     * @return void
     */
    protected function checkAddItempost()
    {
    ...
    protected function getGroupFormFieldConfig($code)
    {
        ...
        $fields = \Arr::get($this->groupDefinitions, $code.'.fields');
    ...
    protected function processGroupMode()
    {
        ...
        foreach ($group as $code => $config) {
            $palette[$code] = [
                ...
                'icon' => \Arr::get($config, 'icon', 'icon-square-o'),
    ...
    {
        return \Arr::get($this->indexMeta, $index.'.groupCode');
    }
    ...
    {
        return \Arr::get($this->groupDefinitions, $groupCode.'.name');
    }
}
